...
| `components/wow/WoWHub.tsx`            | Character list, CheckGrid (28px), M+/boss/custom tasks, Sync/Reset buttons. ilvl `.toFixed(2)`. **After sync, uses `refreshChecklist()` (checklist-only) not `loadChecklist()` to avoid resetting ilvl/stats.** |
